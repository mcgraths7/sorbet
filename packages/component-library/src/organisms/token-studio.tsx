...
function loadBuckets(): Buckets {
  try {
    ...
    if (raw) return { ...emptyBuckets(), ...JSON.parse(raw) };
  } catch {
...
export function TokenStudio({ open, onClose }: TokenStudioProps) {
  ...
  useEffect(() => {
    ...
    for (const def of ALL_TOKENS) base[def.name] = styles.getPropertyValue(`--sb-${def.name}`).trim();
    if (sheet) sheet.textContent = saved;
    setBaseline(base);
  }, [open, mode]);
